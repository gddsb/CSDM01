...
- 工单计划数量**必须为正整数且大于 0**
- 开工后**自动生成报工单**，各工序可自由报工
- **工时计算规则**：按 0.5 小时取值
- 工单完工后，关联记录自动转为只读
- **订单状态联动**：
  - 工单开工后，生产订单状态转为"开工"
  - 工单完工时，若订单累计完工数量 ≥ 计划数量，生产订单状态改为"关闭"
...
- **左侧菜单栏**：固定定位（`position: fixed`），高度 100vh，菜单过长时内部滚动
- **顶部信息栏**：固定定位（`position: fixed`），高度 64px，不随内容滚动
- **内容区域**：独立滚动区域，`flex: 1` 填充剩余空间，`overflow-y: auto`
- **布局结构**：使用 Flexbox 布局，主容器 `height: 100vh; overflow: hidden`，禁止整体滚动
- **收缩适配**：侧边栏收缩时（80px），顶部栏和内容区自动适配左侧间距
...
**列表列**：设备编码、设备名称、设备类型、规格型号、所在产线、状态、操作
...
**列表列（从左到右）**：
不良编码 → 大类名称 → 分类名称 → 不良名称 → 默认单位 → 关联工序 → 不良描述 → 状态 → 操作
...
**表单字段顺序**：
不良编码 → 大类名称 → 分类名称 → 不良名称 → 默认单位 → 关联工序 → 不良描述 → 状态 → 不良图片
