...
- 共16个字段，分4行显示：
  - 第1行：完工单号、订单编号、下发时间、工单状态
  - 第2行：产品料号、产品名称、规格、计划数量
  - 第3行：完工数量（绿色）、不良总数（红色，=计划数量-完工数量，≥0）、合格率（绿色，≤100%）、计划开始
  - 第4行：计划完成、创建时间、创建人
- **合格率计算公式**：`合格率 = MIN(100, 完工数量 / 计划数量 × 100)%`，保留2位小数；计划数量为0时显示 `0.00%`
- **不良总数计算公式**：`不良总数 = MAX(0, 计划数量 - 完工数量)`，确保不为负数

##### 1.5.2 报工单展开汇总区显示规则

- 展开报工单后，顶部显示**一行式汇总数据**（7项，横向排列、不换行）
- 使用 `flex: 1` 均分宽度，`flexWrap: 'nowrap'` 强制不换行
- 每个数据项为浅灰背景（`#fafafa`）圆角块，标签与数值同行显示
