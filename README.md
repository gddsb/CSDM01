...
| 产品检测 | `/quality/product` | 统一检验入口，支持**首件**、**制程**、**成品**、**其它**四种检验类型；主表+子表结构（主表存基本信息，子表存检测项目）；支持新增检验、检测项目维护、报审、详情查看；检验编号按类型两位前缀（SJ首件/ZC制程/CP成品/QT其它）+日期+序号生成；新增记录默认状态"待检"，报审后状态"审核中"不可修改 |
