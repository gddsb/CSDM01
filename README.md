...
| 模块 | 路径 | 功能说明 |
|------|------|---------|
| 任务设置 | `/auto/task-settings` | 自动任务配置管理，支持任务类型、名称、数据源URL、执行参数、启用/停用等配置；内置料品数据同步、客户数据同步、环境监测采集、气象信息抓取、能源采集5个任务 |
...
│   │   ├── seed.ts                   # 数据初始化脚本
│   │   └── migrate.ts                # 数据库列迁移
│   ├── data/                         # 运行时数据目录（开发环境 SQLite 文件）
│   │   └── backups/                  # 开发环境备份文件目录（生产环境使用 /opt/backups/milk-can-mes）
│   ├── uploads/                      # 上传文件目录
│   │   ├── avatars/                  # 用户头像
│   │   ├── tmp/                      # 临时上传目录
│   │   ├── defects/                  # 不良图片
...
> **真实采集任务类型**：`items`（U9料品数据）、`customers`（U9客户数据）、`env_monitor`（环境监测数据）、`weather`（气象信息）、`energy_meter`（能源采集-云集云平台总表有功/无功总电能）。所有采集任务均调用真实外部接口，执行进度实时更新到同步任务日志表。

> **自动任务数据表命名规范**：自动任务模块的所有数据表统一使用 `task_` 前缀（如 `task_setting`、`task_sync_log`、`task_scheduled`、`task_item`、`task_customer`、`task_env_monitor_data`、`task_env_alarm_record`、`task_weather_info`、`task_energy_meter_data`），与系统表（`sys_`）、基础数据表（`bas_`）、生产业务表（`production_`）、质量检验表（`quality_`）等模块前缀保持一致的命名风格。

> **任务设置初始化**：服务启动时自动通过 `initTaskSettings()` 初始化5种默认任务设置记录（料品数据同步、客户数据同步、环境监测采集、气象信息抓取、能源采集），使用 `findOrCreate` 避免重复插入。
