...
| 模块 | 路径 | 功能说明 |
|------|------|---------|
| 生产订单 | `/production/orders` | 订单创建、下发、关闭全生命周期，订单号自动生成；**筛选区支持月份快速选择**（本月/上月/近3个月/近6个月/今年/去年），默认显示本月，查询跨度最大12个月，超3个月显示长查询警告 |
...
| 模块 | 路径 | 功能说明 |
|------|------|---------|
| 来料检验 | `/quality/incoming` | 原材料/外购件入厂检验，主表+子表结构，状态流转与产品检测一致（待检→检验中→审核中→已完成），支持开检/检测/报审/审核全流程，列表"状态"列位于"处理方式"列之后；**筛选区支持月份快速选择**（本月/上月/近3个月/近6个月/今年/去年），默认显示本月，查询跨度最大12个月，超3个月显示长查询警告 |
...
| 表名 | 说明 |
|------|------|
| `u9_task_setting` | 自动任务设置表，存储任务类型、名称、数据源URL、执行参数等配置 |
| `u9_sync_task` | 同步任务日志表，记录数据同步任务的执行状态、进度、步骤日志及结果 |
| `u9_scheduled_task` | 定时任务计划表，存储周期性/定时/单次任务的调度配置、执行时间 |
| `u9_item` | U9料品档案表，存储从U9系统同步的料品基础信息、规格参数及分类信息 |
| `u9_customer` | U9客户档案表，存储从U9系统同步的客户基本信息、分类及信用状态 |
| `env_monitor_data` | 环境监测数据表，存储车间环境监测因子（温湿度等）的实时采集数据 |
| `env_alarm_record` | 环境报警记录表，存储环境监测超限的报警信息、级别及处理状态 |
| `weather_info` | 气象信息表，存储城市/区域的实时天气数据（温度、湿度、大气压等） |
...
### V1.0.1.731 (2026-07-31)

#### P0 - 高优先级

- **生产订单状态筛选默认值调整**：状态快速筛选默认不勾选"关闭"选项，初始仅显示「开立、下发、开工、完工」状态的订单，重置按钮同步恢复为不含"关闭"的默认选项
- **生产订单与报工单状态同步修复**：修复所有报工单已完工但生产订单仍处于"开工"状态的问题。修改 `syncOrderStatus` 联动逻辑：只要该订单下所有报工单均完工，订单自动转为"完工"（status=3）；在此基础上，若完工数量 ≥ 计划数量，再自动转为"关闭"（status=4）
- **生产订单状态兜底检查**：新增已处于"完工"状态的订单，若完工数量达标（≥计划数量），自动转为"关闭"的兜底检查逻辑

#### P1 - 中优先级

- **生产订单页筛选区域说明更新**：README.md 同步更新订单状态联动规则说明文档，确保文档与代码逻辑一致
