...
| 状态值 | 显示名称 | 说明 |
|--------|---------|------|
| 0 | 开始报工 | 报工进行中，子表格可编辑，不可新增下一张报工单 |
| 1 | 结束报工 | 报工已完成，子表格只读，可新增下一张报工单 |
...
- 关键字（设备编号/名称/型号模糊搜索）
- 设备类型（下拉，来自现有数据去重）
- 状态（多选勾选框：运行/维修/停用，默认勾选全部状态）
- 是否特种设备（是/否）
- 状态筛选框宽度为 200px，其他筛选框宽度为 150px，所有控件放在一行
...
**列表列（从左到右）**：
不良编码 → 检验类型 → 不良类型 → 不良项目 → 默认单位 → 关联工序 → 不良描述 → 状态 → 操作
...
**关联工序规则**：
- 当检验类型为「来料检验类型」时，**关联工序自动清空且禁用**（不可选）
- 当检验类型为「制程检验类型」时，关联工序列表可用，可多选工序
- 切换检验类型时自动清空已选关联工序
...
- 编码格式：三段式编码 `检验类型缩写-不良类型缩写-两位流水码`
  - 检验类型缩写：来料检验类型→IC（Incoming），制程检验类型→PC（Process）
  - 不良类型缩写：来料不良→MAT，制程不良→PRC，检验报废→SCP
  - 两位流水码：按"检验类型+不良类型"组合分别编号（01、02、03...）
- 示例：
  - IC-MAT-01（来料检验类型-来料不良-第1条）
  - PC-PRC-01（制程检验类型-制程不良-第1条）
  - PC-SCP-01（制程检验类型-检验报废-第1条）
- 选择不良类型后自动生成对应编码
- 保存时若编码为空，会兜底重新获取编码后再提交
- 编辑时编码可修改

**表单字段顺序**：
不良编码 → 检验类型 → 不良类型 → 不良项目 → 可选单位 → 默认单位 → 关联工序 → 不良描述 → 状态 → 不良图片

**可选单位与默认单位交互规则**：
- **可选单位**：多选勾选框（小片、罐、大张），最少有一个选中项
- **默认单位**：下拉选择列表，数据来源为可选单位中选中的项目
- 用户改变可选单位选中项时，若默认单位被取消勾选则自动清空默认单位选中项
- 可选单位若为唯一选中项时，默认单位自动选中该项
...
- 关键字（订单号/料品名模糊搜索）
- 料号（模糊搜索）
- 状态多选框（开立/下发/完工，默认选中开立、下发，宽度 200px）
- 计划时间范围（开始日期~结束日期）
...
**业务模型**：
- **生产报工单**是**生产工单**的下级独立单据
- 一个生产工单可对应多张生产报工单（适用于多日生产场景）
- 子表格数据（不良记录、物料记录、异常工时、人员工时）均挂在具体报工单下
...
| 层级 | 区域 | 说明 |
|------|------|------|
| 第一级 | 生产工单选择 | 选择要报工的生产工单 |
| 第二级 | 报工单管理 | 选择/新增报工单，显示报工单基本信息及状态切换 |
| 第三级 | 子表格数据 | 5 个 Tab：生产不良记录、生产物料记录、检验报废记录、异常工时记录、人员工时记录 |
...
**报工单创建规则**：
- 手动点击「新增报工单」按钮创建
- 一个生产工单**同一天只能创建一个**报工单
- 上一条报工单处于「开始报工」状态时，不允许新增报工单
- 工单必须为「开工」状态才能创建报工单
...
**报工单号规则**：`PR` + `YYMMDD` + 3位流水号（示例：`PR260715001`）

**子表格说明**：
- **生产不良记录**：登记生产过程中的不良项目及数量
- **生产物料记录**：登记物料投入/产出情况
- **检验报废记录**：登记检验报废项目
- **异常工时记录**：登记异常停机工时
- **人员工时记录**：登记生产人员工时
...
- **工单 - 生产报工单**：一对多（WorkOrder.hasMany(ProcessReport)）
- **生产报工单 - 工序不良记录**：一对多（ProcessReport.hasMany(ProcessDefect)）
- **生产报工单 - 工序物料记录**：一对多（ProcessReport.hasMany(ProcessMaterial)）
- **生产报工单 - 异常工时记录**：一对多（ProcessReport.hasMany(ProcessException)）
- **生产报工单 - 人员工时记录**：一对多（ProcessReport.hasMany(ManpowerRecord)）
...
- **服务器状态**：前端开发服务器（端口 5173）、后端服务（端口 3001）运行状态检测（在线/离线）
