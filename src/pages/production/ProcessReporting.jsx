...
export default function ProcessReporting() {
    ...
    const run = async () => {
      try {
        const res = await api.get('/production/process-reports', {
          params: { work_order_id: selectedWO.work_order_id, page: 1, pageSize: 1000 },
        })
        if (cancelled) return
        setReports(res.data || [])
      } catch (err) {
        if (!cancelled) message.error(err.message || '获取报工记录失败')
      }
    }
    run()
    return () => { cancelled = true }
  ...
  // 选中工序时加载不良记录
  useEffect(() => {
    if (!selectedWO || !selectedProcessId) {
      setProcessDefects([])
      return
    }
    let cancelled = false
    const run = async () => {
      try {
        const res = await api.get('/production/process-defects', {
          params: { work_order_id: selectedWO.work_order_id, process_id: selectedProcessId, page: 1, pageSize: 1000 },
        })
        if (cancelled) return
        setProcessDefects(res.data || [])
      } catch (err) {
        if (!cancelled) message.error(err.message || '获取不良记录失败')
      }
    }
    run()
    return () => { cancelled = true }
  }, [selectedWO, selectedProcessId])

  // 根据不良分类和已保存的不良记录，生成左右两侧数据
  const defectItems = useMemo(() => {
    ...
    const existingMap = {}
    processDefects.forEach(d => { existingMap[d.defect_name] = d })
  ...
  useEffect(() => {
    setProcessDefectData(defectItems.process.map((item, idx) => ({ ...item, key: `p-${idx}` })))
    setMaterialDefectData(defectItems.material.map((item, idx) => ({ ...item, key: `m-${idx}` })))
  }, [defectItems])

  // 工序下拉选项
  const processOptions = useMemo(() => {
    if (!selectedWO) return []
    return reports.map(r => ({ label: r.process_name, value: r.process_id }))
  }, [selectedWO, reports])
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      setEditing(null)
      // 刷新报工记录
      const reportRes = await api.get('/production/process-reports', {
        params: { work_order_id: selectedWO.work_order_id, page: 1, pageSize: 1000 },
      })
  ...
  const handleSaveDefects = async () => {
    ...
    try {
      ...
      const payload = { work_order_id: selectedWO.work_order_id, process_id: selectedProcessId, items: allItems }
      const res = await api.post('/production/process-defects/batch-save', payload)
      message.success(res.message || '保存成功')
      // 刷新不良记录
      const defectRes = await api.get('/production/process-defects', {
        params: { work_order_id: selectedWO.work_order_id, process_id: selectedProcessId, page: 1, pageSize: 1000 },
      })
  ...
  const woColumns = [
    ...
    { title: '状态', dataIndex: 'status', key: 'status', width: 80, render: v => <Tag color={woStatusColorMap[v] || 'default'}>{v}</Tag> },
  ]
  ...
  const reportColumns = [
    ...
    { title: '投入工时', key: 'input_hours', width: 100, render: () => '-' },
  ...
  return (
    <>
      <ThreeSectionPage
        ...
        stats={stats}
        actions={<ActionButtons hasAdd={false} hasExport={false} />}
        table={
          <div>
              ...
              <>
                <Divider orientation="left" style={{ margin: '12px 0' }}>
                  <span style={{ fontWeight: 600 }}>
                    ...
                    <Tag color={woStatusColorMap[selectedWO.status]} style={{ marginLeft: 8 }}>{selectedWO.status}</Tag>
                  </span>
                </Divider>
      ...
      >
        ...
        <Form form={form} layout="vertical" className="compact-form" preserve={false}>
          ...
          <Form.Item label="报工时间" name="report_time">
            <DatePicker showTime format="YYYY-MM-DD HH:mm:ss" style={{ width: '100%' }} placeholder="选择报工时间" />
          </Form.Item>
        </Form>
      </Modal>

      {/* 查看报工详情 */}
      <Drawer title="报工详情" open={!!viewRecord} onClose={() => setViewRecord(null)} width={560}>
        {viewRecord && (
          ...
          <Form.Item label="不良项目" name="defect_type_id" rules={[{ required: true, message: '请选择不良项目' }]}>
            <Select
              ...
              onChange={(value) => {
                ...
                if (d) addDefectForm.setFieldsValue({ defect_name: d.defect_name, unit: d.defect_unit })
              }}
            />
          </Form.Item>
