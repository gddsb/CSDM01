...
export default function ManpowerRecord() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedWO) fetchDetail(selectedWO.work_order_id)
      setQuery(q => ({ ...q }))
    } catch (e) {
  ...
  const calcTotalHours = (record) => {
    ...
    const totalP = record.total_people || 0
    return diff > 0 ? Number((diff * totalP).toFixed(2)) : 0
  }
  ...
  const columns = [
    ...
    {
      title: '总工时(h)', key: 'total_hours', width: 120, align: 'right',
      render: (_, r) => Number(calcTotalHours(r)).toFixed(2),
    },
  ...
  return (
    <>
      ...
      {/* 查看详情抽屉 */}
      <Drawer title="人员记录详情" open={!!currentRecord} onClose={() => setCurrentRecord(null)} width={480}>
        {currentRecord && (
