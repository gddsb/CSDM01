...
const tableCategoryMap = {
  ...
  quality_incoming_inspection: { category: '业务表', purpose: '来料检验子表，存储来料检验的检验项目、标准值、检测值、判定结果等详细信息' },
  quality_incoming_inspection_item: { category: '业务表', purpose: '来料检验主表，存储来料检验基本信息，含检验编号、关联检验标准、供应商、批次、结果、状态等' },
...
const columnCommentMap = {
  ...
  quality_inspection_standard: {
    ...
    standard_type: '标准类型：通用标准/专用标准/临时标准',
    customer_code: '客户编码',
    material_id: '参照料品ID',
    material_name: '料品名称（冗余）',
    version_no: '版本号（默认V1）',
