...
// ============ 验证码识别方案 ============
import { CAPTCHA_SCHEMES, testCaptchaScheme, fetchEnergyCaptcha, loginEnergyWithCaptcha } from '../services/energyMeterCollector.js'
...
// ============ 能源采集：获取验证码图片 ============
export const getEnergyCaptcha = async (req, res) => {
  try {
    const result = await fetchEnergyCaptcha()
    return success(res, result, '获取验证码成功')
  } catch (err: any) {
    console.error('[getEnergyCaptcha] 失败:', err)
    return fail(res, err.message || '获取验证码失败', ErrorCode.SYSTEM_ERROR)
  }
}

// ============ 能源采集：使用验证码登录获取Token ============
export const loginEnergyByCaptcha = async (req, res) => {
  try {
    const { loginName, password, keyStr, code } = req.body
    if (!loginName || !password || !keyStr || !code) {
      return fail(res, '缺少必要参数：loginName, password, keyStr, code', ErrorCode.PARAM_ERROR)
    }
    const result = await loginEnergyWithCaptcha(loginName, password, keyStr, code)
    if (result.success && result.token) {
      return success(res, { token: result.token }, '登录成功，已获取访问令牌')
    }
    return fail(res, result.error || '登录失败', ErrorCode.BIZ_ERROR)
  } catch (err: any) {
    console.error('[loginEnergyByCaptcha] 失败:', err)
    return fail(res, err.message || '登录失败', ErrorCode.SYSTEM_ERROR)
  }
}
