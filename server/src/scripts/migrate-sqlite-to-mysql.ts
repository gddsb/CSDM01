...
function cleanDate(v: any): string | null {
  if (!v) return null
  let s = String(v)
  s = s.replace(/\.\d{3,6}\s*\+\d{2}:\d{2}$/, '').replace(/\s*\+\d{2}:\d{2}$/, '').replace(/\.\d{6}$/, '').replace(/\.\d{3}$/, '')
  return s
}
...
async function main() {
  ...
  for (const s of settings) {
    const [existing]: any = await mysqlConn.execute(
      'SELECT setting_id FROM u9_task_setting WHERE task_type = ?', [s.taskType]
    )
  ...
  for (const s of scheds) {
    const [existing]: any = await mysqlConn.execute(
      'SELECT schedule_id FROM u9_scheduled_task WHERE schedule_biz_id = ?', [s.scheduleId]
    )
  ...
  for (const t of tasks) {
    const [existing]: any = await mysqlConn.execute(
      'SELECT task_id FROM u9_sync_task WHERE task_biz_id = ?', [t.taskId]
    )
  ...
  for (const c of customers) {
    const [existing]: any = await mysqlConn.execute('SELECT customer_id FROM u9_customer WHERE customer_code = ?', [c.customerCode])
  ...
  for (const m of monitors) {
    const [existing]: any = await mysqlConn.execute('SELECT monitor_id FROM env_monitor_data WHERE factor_id = ? AND collect_time = ?', [m.factorId, cleanDate(m.collectTime)])
  ...
  for (const a of alarms) {
    const [existing]: any = await mysqlConn.execute('SELECT alarm_id FROM env_alarm_record WHERE factor_id = ? AND alarm_time = ?', [a.factorId, cleanDate(a.alarmTime)])
