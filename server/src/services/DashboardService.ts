/**
 * DashboardService — 5 个看板（overview/trend/production/quality/management）
 * 从 modules/auto/DashboardController.ts（580 行）抽取
 * 统一复用 AutoTaskService.calcDewPoint，不再内联重复定义
 */
...
import { calcDewPoint } from './AutoTaskService.js'
import { formatDateTime, formatDate, nowBeijingStr, nowBeijingDate } from '../utils/date.js'
...
export async function dashboardOverview(): Promise<any> {
  ...
  try {
    const latestWeather = await WeatherInfo.findOne({ order: [['weather_time', 'DESC']], raw: true }) as any
  ...
  const todayStart = nowBeijingDate(); todayStart.setHours(0, 0, 0, 0)

  const [totalAlarms, unhandledAlarms, todayAlarms, recentAlarms] = await Promise.all([
    EnvAlarm.count(),
    EnvAlarm.count({ where: { is_handled: 0 } }),
    EnvAlarm.count({ where: { alarm_time: { [Op.gte]: todayStart } } }),
    EnvAlarm.findAll({ order: [['alarm_time', 'DESC']], limit: 15, raw: true }),
  ])
  ...
  return {
    ...
    lastUpdate: (latestBatch as any[])[0]?.collect_time || null,
  }
}

// ============ 趋势数据（最近12小时，10分钟间隔） ============

export async function dashboardTrend(): Promise<any> {
  ...
  function bucketAvg(recs: Rec[], target: Date, windowMs: number = 10 * 60 * 1000): number | null {
    ...
    let sum = 0; let count = 0; let fallback: Rec | null = null; let fallbackDiff = Infinity
    for (const r of recs) {
      const diff = Math.abs(r.time.getTime() - targetTs)
      if (diff <= windowMs / 2) { sum += r.value; count++ }
      if (diff < fallbackDiff) { fallback = r; fallbackDiff = diff }
    }
  ...
  for (const area of areas) {
    ...
    const temp: (number | null)[] = []; const hum: (number | null)[] = []; const dew: (number | null)[] = []
    for (const mark of slotMarks) {
      const tv = bucketAvg(tempRecs, mark)
      const hv = bucketAvg(humRecs, mark)
      temp.push(tv); hum.push(hv); dew.push(tv !== null && hv !== null ? calcDewPoint(tv, hv) : null)
    }
  ...
  return { hours: 12, intervalMinutes: 10, times, series }
}

// ============ 生产实时看板 ============

export async function productionDashboard(): Promise<any> {
  const todayStart = nowBeijingDate(); todayStart.setHours(0, 0, 0, 0)
  const thirtyDaysAgo = new Date(todayStart.getTime() - 30 * 24 * 60 * 60 * 1000)
  const now = nowBeijingDate()

  const [productionLines, devices, processes, materials, orders, workOrders, processReports] = await Promise.all([
    ProductionLine.findAll({ order: [['sort_order', 'ASC']], raw: true }),
    Device.findAll({ raw: true }),
    Process.findAll({ order: [['sort_order', 'ASC']], raw: true }),
    Material.findAll({ limit: 50, raw: true }),
    Order.findAll({
      where: { [Op.or]: [
      ...
      order: [['created_at', 'DESC']], limit: 200, raw: true,
    }),
    ReportOrder.findAll({
      where: { [Op.or]: [
      ...
      order: [['report_time', 'DESC']], limit: 500, raw: true,
    }),
    ReportProcess.findAll({
      where: { created_at: { [Op.gte]: todayStart } },
      order: [['created_at', 'DESC']], limit: 2000, raw: true,
    }),
  ])
  ...
  const processReportOrders = reportOrderIds.length > 0
    ? await ReportOrder.findAll({ where: { report_order_id: { [Op.in]: reportOrderIds } }, raw: true })
    : []
  const roMap = new Map()
  ...
  const processReportsWithQty: any[] = processReports.map(pr => {
    ...
    return {
      ...
      defect_material: 0, defect_process: 0, defect_scrap: 0,
    }
  })

  const defects = await ProcessDefect.findAll({ where: { record_time: { [Op.gte]: todayStart } }, raw: true })
  defects.forEach(df => {
  ...
  const reportOrders30d = await ReportOrder.findAll({
    ...
    order: [['report_time', 'ASC']], limit: 10000, raw: true,
  })
  ...
  dateList.forEach(dateStr => {
    ...
    lineNames.forEach(ln => { item[ln] = dailyOutputMap[dateStr]?.[ln] || 0 })
    dailyTrend.push(item)
  })

  const energyData30d = await EnergyMeterData.findAll({
    where: { reading_date: { [Op.gte]: thirtyDaysAgo } },
    order: [['reading_date', 'ASC']], raw: true,
  })
  const dailyEnergyMap: Record<string, number> = {}
  energyData30d.forEach(e => {
    const t = e.reading_date; if (!t) return
    dailyEnergyMap[formatDate(t)] = (dailyEnergyMap[formatDate(t)] || 0) + Number(e.forward_active_energy || 0)
  })
  ...
  const defects30d = await ProcessDefect.findAll({
    where: { record_time: { [Op.gte]: thirtyDaysAgo } }, raw: true,
  })
  ...
  defects30d.forEach(df => {
    ...
    if (!processDefectMap[pname]) processDefectMap[pname] = { name: pname, material: 0, process: 0, scrap: 0, total: 0 }
    const qty = Number(df.quantity || 0)
  ...
  const processDefectList = Object.values(processDefectMap).filter(p => p.total > 0).sort((a, b) => b.total - a.total)

  return {
    productionLines, devices, processes, materials, orders, workOrders, processReports: processReportsWithQty,
    dailyTrend, dailyEnergy, processDefectList,
    queryTime: nowBeijingStr(), activeDate: formatDate(todayStart),
  }
}

// ============ 质量检测中心看板 ============

export async function qualityDashboard(): Promise<any> {
  const todayStart = nowBeijingDate(); todayStart.setHours(0, 0, 0, 0)
  const sevenDaysAgo = new Date(todayStart.getTime() - 7 * 24 * 60 * 60 * 1000)

  const [incomingInspections, productInspections, microbeInspections] = await Promise.all([
    IncomingInspection.findAll({ where: { created_at: { [Op.gte]: sevenDaysAgo } }, order: [['created_at', 'DESC']], limit: 500, raw: true }),
    ProductInspection.findAll({ where: { created_at: { [Op.gte]: sevenDaysAgo } }, order: [['created_at', 'DESC']], limit: 500, raw: true }),
    MicrobeInspection.findAll({ where: { created_at: { [Op.gte]: sevenDaysAgo } }, order: [['created_at', 'DESC']], limit: 500, raw: true }),
  ])

  const finishedInspections = productInspections.filter(i => i.inspection_type === '成品' || i.inspection_type === '制程')
  const envInspections = microbeInspections.filter(i => i.object_type !== '来料检验')
  ...
  return {
    ...
    complaints: [], instruments, inspectionStandards, materials,
  }
}

// ============ 经营管理中心看板 ============

export async function managementDashboard(): Promise<any> {
  const todayStart = nowBeijingDate(); todayStart.setHours(0, 0, 0, 0)
  const monthStart = new Date(todayStart.getFullYear(), todayStart.getMonth(), 1)
  const sixMonthsAgo = new Date(todayStart); sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 5); sixMonthsAgo.setDate(1)
  ...
  return {
    orders, workOrders, processReports: processReportsWithQty, productionLines, devices,
    incomingInspections, finishedInspections, microbeInspections, envInspections,
    complaints: [], instruments, materials, customers, inspectionStandards,
  }
}
