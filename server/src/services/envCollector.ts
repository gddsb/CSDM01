...
export class EnvCollector {
    ...
    for (const device of devices) {
      ...
      for (const item of device.dataItem) {
        for (const reg of item.registerItem) {
          records.push({
            factor_id: `${device.deviceAddr}_${item.nodeId}_${reg.registerId}`,
            device_addr: device.deviceAddr,
            device_name: device.deviceName,
            node_id: item.nodeId,
            register_id: reg.registerId,
            factor_name: reg.registerName,
            value: reg.value,
            raw_data: reg.data,
            unit: reg.unit,
            coefficient: null,
  ...
  async collectAndSave(): Promise<{ saved: number; devices: number; alarms: number }> {
    ...
    for (const r of records) {
      if (coeffMap.has(r.factor_id)) {
        r.coefficient = coeffMap.get(r.factor_id);
      }
    }
